...
local stan_systemu = {
  ...
  botserver = {
    mana_sojusznikow = {},
    ostatnie_potowanie = {},
    ostatnia_wysylka_many = 0,
    ostatnia_proba_polaczenia = 0,
    ostatnia_mana = -1,
    listenery_zainstalowane = false,
    standard_listenery_zainstalowane = false,
    kanal_polaczenia = nil
  }
}
...
local function oznaczCzlonkaBotServera(nick, vocation)
  ...
  local wartosc = vocation or true
  if type(vBot) == "table" then
    vBot.BotServerMembers = vBot.BotServerMembers or {}
    vBot.BotServerMembers[clean] = wartosc
  end

  BotServerMembers = BotServerMembers or {}
  BotServerMembers[clean] = wartosc
end

local function czyCzlonekBotServera(nick)
  local clean = przytnij(nick)
  if clean == "" then return false end
...
local function znajdzGraczaPoNicku(nick)
  if getCreatureByName then
    local creature = getCreatureByName(nick)
    if creature then return creature end
  end
  if getPlayerByName then
    return getPlayerByName(nick)
  end
  return nil
end

local function ustawKonfiguracjeBotServer()
  storage.BotServerChannel = konfiguracja.botserver.kanal
  storage.BOTserver = storage.BOTserver or {}
  storage.BOTserver.enabled = true
  storage.BOTserver.manaInfo = true
  storage.BOTserver.vocation = true

  if botServerWindow and botServerWindow.Data and botServerWindow.Data.Channel then
    botServerWindow.Data.Channel:setText(konfiguracja.botserver.kanal)
  end
  if botServerWindow and botServerWindow.enabled then
    botServerWindow.enabled:setOn(true)
  end
  if botServerWindow and botServerWindow.Features then
    if botServerWindow.Features.Feature1 then botServerWindow.Features.Feature1:setOn(true) end
    if botServerWindow.Features.Feature3 then botServerWindow.Features.Feature3:setOn(true) end
  end
end

local function zainstalujListeneryRezoriaBotServer()
  if stan_systemu.botserver.listenery_zainstalowane then return end
  if type(BotServer) ~= "table" or not BotServer._websocket or not BotServer.listen then return end

  BotServer.listen("rezoria_mana", function(nadawca, message)
    local clean = przytnij(nadawca)
    if clean == "" or czyToJa(clean) then return end

    local mana, vocation
    if type(message) == "table" then
      mana = tonumber(message.mana)
      vocation = message.vocation
    else
      mana = tonumber(message)
    end
    if not mana then return end

    mana = math.max(0, math.min(100, mana))
    stan_systemu.botserver.mana_sojusznikow[normalizeName(clean)] = {
      nick = clean,
      mana = mana,
      vocation = vocation,
      czas = now
    ...
    oznaczCzlonkaBotServera(clean, vocation)

    local creature = znajdzGraczaPoNicku(clean)
    if creature and creature.setManaPercent then
      creature:setManaPercent(mana)
    end
  end)

  stan_systemu.botserver.listenery_zainstalowane = true
end

local function polaczBotServerRezoria()
  if type(BotServer) ~= "table" or not BotServer.init then return false end
  if g_game and g_game.isOnline and not g_game.isOnline() then return false end

  local poprzedni_kanal = tostring(storage.BotServerChannel or "")
  BotServer.url = konfiguracja.botserver.url
  ustawKonfiguracjeBotServer()

  local inny_kanal = poprzedni_kanal ~= konfiguracja.botserver.kanal
    or stan_systemu.botserver.kanal_polaczenia ~= konfiguracja.botserver.kanal

  if BotServer._websocket and inny_kanal and BotServer.terminate then
    BotServer.terminate()
    stan_systemu.botserver.listenery_zainstalowane = false
    stan_systemu.botserver.standard_listenery_zainstalowane = false
    stan_systemu.botserver.kanal_polaczenia = nil
    stan_systemu.botserver.ostatnia_proba_polaczenia = 0
  end

  if not BotServer._websocket then
    local ostatnia_proba = stan_systemu.botserver.ostatnia_proba_polaczenia or 0
    if ostatnia_proba > 0 and now - ostatnia_proba < konfiguracja.botserver.reconnect_ms then return false end
    stan_systemu.botserver.ostatnia_proba_polaczenia = now
    stan_systemu.botserver.listenery_zainstalowane = false
    stan_systemu.botserver.standard_listenery_zainstalowane = false
    local lp = pobierzLokalnegoGracza()
    local moja_nazwa = (type(name) == "function" and name()) or (lp and lp:getName()) or ""
    BotServer.init(moja_nazwa, konfiguracja.botserver.kanal)
  end

  if BotServer._websocket then
    stan_systemu.botserver.kanal_polaczenia = konfiguracja.botserver.kanal
    zainstalujListeneryRezoriaBotServer()
    if not stan_systemu.botserver.standard_listenery_zainstalowane and type(initBotServerListenFunctions) == "function" then
      initBotServerListenFunctions()
      stan_systemu.botserver.standard_listenery_zainstalowane = true
    end
  end
...
local function wyslijStatusManyBotServer()
  if type(BotServer) ~= "table" or not BotServer._websocket or not BotServer.send then return false end
  if now - (stan_systemu.botserver.ostatnia_wysylka_many or 0) < konfiguracja.botserver.wysylka_many_ms then return false end

  local mana = manapercent()
  stan_systemu.botserver.ostatnia_wysylka_many = now
  BotServer.send("rezoria_mana", { mana = mana, vocation = stan_systemu.zawod.moj or "" })

  if mana ~= stan_systemu.botserver.ostatnia_mana then
    stan_systemu.botserver.ostatnia_mana = mana
    BotServer.send("mana", { mana = mana })
  end

  return true
end

macro(konfiguracja.botserver.wysylka_many_ms, function()
  polaczBotServerRezoria()
  wyslijStatusManyBotServer()
end)
...
-- 8. healing
-- Rozdzielone makra: SIO odpowiada za heal po HP, a POT KOGOS najpierw uzywa BotServera, potem p/x jako fallback.
...
local function pobierzManeSojusznikaBotServer(creature)
  if not creature or not creature.getName then return nil end
  local wpis = stan_systemu.botserver.mana_sojusznikow[normalizeName(creature:getName())]
  if not wpis then return nil end
  if now - (wpis.czas or 0) > konfiguracja.botserver.dane_ttl_ms then return nil end
  return tonumber(wpis.mana)
end

local function czyMoznaPotowacPrzezBotServer(creature)
...
local function wybierzSojusznikaBotServerDoPota(myPos)
  local wybrany, wybrana_mana
  local widoczni = getSpectators(myPos, false, true, 1, 1, 1, 1)

  for _, creature in ipairs(widoczni) do
    if czyMoznaPotowacPrzezBotServer(creature) and czyStoiObok(creature, myPos) then
      local mana = pobierzManeSojusznikaBotServer(creature)
      if mana and mana < konfiguracja.botserver.prog_many then
        local key = normalizeName(creature:getName())
        local ostatnio = stan_systemu.botserver.ostatnie_potowanie[key] or 0
        if now - ostatnio >= konfiguracja.botserver.pot_cooldown_ms then
          if not wybrana_mana or mana < wybrana_mana then
            wybrany = creature
            wybrana_mana = mana
          end
        end
  ...
  return wybrany, wybrana_mana
end

local function pobierzItemPotaBotServer()
  local zawod = stan_systemu.zawod.moj
  if (zawod == "ED" or zawod == "MS") and manapercent() > konfiguracja.botserver.wlasna_mana_min then
    return konfiguracja.botserver.item_many
  end
  if zawod == "RP" and hppercent() > konfiguracja.botserver.wlasne_hp_min then
    return konfiguracja.botserver.item_rp
  end
  return nil
end

local function wykonajPotowanieBotServer()
  if type(BotServer) ~= "table" or not BotServer._websocket then return false end

  local lp = pobierzLokalnegoGracza()
  if not lp then return false end

  local item_id = pobierzItemPotaBotServer()
  if not item_id then return false end

  local target = wybierzSojusznikaBotServerDoPota(lp:getPosition())
  if not target then return false end

  useWith(item_id, target)
  stan_systemu.botserver.ostatnie_potowanie[normalizeName(target:getName())] = now
  return true
end
...
macro(200, "POT KOGOS", function()
  if wykonajPotowanieBotServer() then return end
  if type(BotServer) ~= "table" or not BotServer._websocket then
...
schedule(1000, polaczBotServerRezoria)
aktualizujLiderow()
aktualizujZielonyChat()
aktualizujSojusznikow()
aktualizujWrogow()
